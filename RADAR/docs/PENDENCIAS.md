...
1. **Link de checkout Hotmart** — todos os `<a href="#">` de CTA precisam do link real
2. **Logo final** — `assets/img/logo.svg` e `favicon.svg` são placeholders geométricos (círculos + ponto vermelho). Substituir pelos SVGs finais (horizontal + 1:1) quando enviados.
3. **`og-image.png`** — placeholder gerado programaticamente. Substituir por arte final 1200×630.
4. **Paleta** — usando a nova paleta azul do brief (`2563EB/3B82F6/60A5FA/334155`). Decisão de fundir com a paleta neutra anterior (`FAFAFA/1A1A1A/6B7280`) mantida em aberto — ver `variables.css`.
